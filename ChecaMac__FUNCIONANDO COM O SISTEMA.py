...
def run_bot_milhao():
    try:
        # Executa BotMilhao.py no mesmo terminal
        os.system(f"python {os.path.join(os.path.dirname(__file__), 'BotMilhao.py')}")
    except Exception as e:
        print(f"Erro ao iniciar BotMilhao.py: {e}")
...
def main():
    ...
    if check_expiration():
        ...
        run_bot_milhao()  # Executa BotMilhao.py se o período de avaliação ainda é válido
    else:
        ...
        if mac_address:
            if check_mac_in_website(mac_address):
                ...
                print("SISTEMA INICIADO. BOA SORTE")
                run_bot_milhao()  # Executa BotMilhao.py se o MAC estiver liberado
            else:
